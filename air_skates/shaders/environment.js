...
const environmentFragment = `
  ...
  void main() {
    ...
    vec4 color1 = vec4(0.55, 0.4, 1. , 1.0); // DB0C36
    vec4 color2 = vec4(1. , 0.8, 0.3, 1.0); // FDAB89
    vec4 color3 = vec4(0.1, 1., 0.3, 1.0); // 7EB1A8
  ...
  vec2 waveUv = vUv - 0.1;
  waveUv.x += time/3.;
  float wave = step(.09, abs(sin(waveUv.x * 12.) / 24. + .5 - waveUv.y));
  finalColor.r *= sin(time*2.)/1.5+1.;
  finalColor.g /= cos(time*2.)/1.5+1.;
  finalColor.b *= cos(time*2.)/1.5+1.;

  finalColor.r -= 0.1;
  finalColor.g += (1. - finalColor.r)/5.;
  finalColor.b += (1. - finalColor.r)/5.;
  
  vec4 displaceColor = vec4(finalColor.bgr , 1.);
  displaceColor *= (1. - wave);
  finalColor *= wave;
  // finalColor.rgb *= sin( time );
  finalColor += displaceColor;

  // vec3 normalColor = vNormal;
  // normalColor.r *= sin(time*2.);
  // normalColor.g *= cos(time*2.);
  // normalColor.b *= cos(time*2.);
...
export { environmentVertex, environmentFragment }
